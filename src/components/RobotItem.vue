<template>
    <picture className="flex justify-center items-center h-36 w-36 relative -top-5 -left-6 -mb-16 -mr-4">
...
import { computed } from 'vue';
    ...
    const props = defineProps({
        name: {
            type: String,
            validator: (val) => ['smile', 'normal'].includes(val)
        },
        rotate: {
            type: Number,
            validator: (val) => [-12, 0, 12].includes(val)
        }
    });
    ...
    const imageName = props.name || getRandomVal(['smile', 'normal']);
